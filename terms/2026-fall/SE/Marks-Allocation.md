# Marks Allocation Strategy

## CS-326 — Software Engineering

| Component | Weight | Strategy |
|-----------|--------|----------|
| COCOMO numericals | ~15% | Memorize coefficients, drill 10 problems |
| DFD/UML diagrams | ~15% | Practice with modeling tools |
| Process models | ~15% | Comparison table memorization |
| Testing (cyclomatic) | ~10% | Formula + control flow graphs |
| Theory | ~25% | Structure: definition + explanation + example |
| Lab project | ~20% | Group management protocol, start early |

> **Study-effort vs exam weight:** the rows above are study-effort allocations (theory rows sum to 80%, lab 20%). The official exam split is **60% theory exam + 40% lab (project + reports + viva)** per `Week-by-Week-Narrative.md` line 11 — the 40% lab is the official risk component.

